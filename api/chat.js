...
GLD ETF last session: ${(sq.find(x=>x.s==='GLD')?.dp > 0 ? '+' : '')}${sq.find(x=>x.s==='GLD')?.dp?.toFixed(2)||'0'}% | USO ETF: ${(sq.find(x=>x.s==='USO')?.dp > 0 ? '+' : '')}${sq.find(x=>x.s==='USO')?.dp?.toFixed(2)||'0'}% | SLV ETF: ${(sq.find(x=>x.s==='SLV')?.dp > 0 ? '+' : '')}${sq.find(x=>x.s==='SLV')?.dp?.toFixed(2)||'0'}% — NOTE: use COMMODITIES (live spot) section above for actual gold/oil prices
